# PLOT_PATH = "/hpc/users/pateln54/chdiTrios/ME_DEL/xhmm/xhmm_hg19/possible_hits_plots"
# JOB_PREFICES = "/hpc/users/pateln54/chdiTrios/ME_DEL/xhmm/xhmm_hg19/hg19.all.xhmm"
# JOB_TARGETS_TO_GENES = "/hpc/users/pateln54/chdiTrios/ME_DEL/xhmm/xhmm_hg19/hg19.annotated_targets.refseq.loci"
# XHMM_plots(PLOT_PATH, JOB_PREFICES, JOB_TARGETS_TO_GENES)


function (PLOT_PATH, JOB_PREFICES, JOB_TARGETS_TO_GENES = NULL,
    SAMPLE_FEATURES = NULL, SQ_THRESH = 60, NUM_ADD_TARGS = 2,
    PLOT_READ_DEPTHS = TRUE, PLOT_PC_CORRS = TRUE, PLOT_ALL_CNVS = TRUE,
    USE_XCNV_TO_PLOT = NULL, INCLUDE_PEDIGREE_SAMPLES = NULL,
    PLOT_ONLY_PNG = TRUE, LIMIT_MEMORY = FALSE)
{
    ...
    READ_DEPTH_PIPELINE = c("RD", "filtered_centered_RD", "PCA_NORMALIZED",
        "PCA_NORM_Z_SCORES")
    READ_DEPTH_PIPELINE_SHORT_NAMES = c("DoC read-depth", "Target centering",
        "PCA", "Sample z-score")
    READ_DEPTH_PIPELINE_FULL_NAMES = c("Unnormalized read depth",
        "Target-centered read depth", "PCA-normalized read depth",
        "Sample Z-score of PCA-normalized read depth")
    EXCLUDE_LARGE_MATRICES = c()
    # no memory limit so ignore
    # if (LIMIT_MEMORY) {
    #     EXCLUDE_LARGE_MATRICES = c("filtered_centered_RD", "PCA_NORMALIZED",
    #         "PCA_NORM_Z_SCORES", "RD_SAME_FILTERED", "DIP_POST",
    #         "DEL_POST", "DUP_POST")
    #     if (PLOT_READ_DEPTHS) {
    ...
    #         if (!PLOT_ONLY_PNG) {
    #             EXCLUDE_LARGE_MATRICES = setdiff(EXCLUDE_LARGE_MATRICES,
    #               c("filtered_centered_RD", "PCA_NORMALIZED",
    #                 "DIP_POST", "DEL_POST", "DUP_POST"))
    #         }
    #     }
    ...
    if (!is.null(JOB_TARGETS_TO_GENES)) {
        for (job in JOBS) {
            if (!is.null(JOB_TARGETS_TO_GENES[job])) {
                writeLines(paste("Loading genes for ", job, " targets",
                  sep = ""))
                xhmm_data[[job]][["TARGETS_TO_GENES"]] = loadTargetsToGenes(JOB_TARGETS_TO_GENES[job],
                  CHROMOSOMES_START_WITH_CHR = xhmm_data[[job]][["CHROMOSOMES_START_WITH_CHR"]])
            }
        }
    }
    for (job in JOBS) {
    ...
    if (PLOT_PC_CORRS) {
        for (job in JOBS) {
            ...
            rm(PC_LOADINGS)
            filteredRD = xhmm_data[[job]][["RD"]][PC_LOADINGS_samps,
                PC_targs]
        ...
        for (job in JOBS) {
            ...
            PC_t = t(PC)
            rm(PC)
            vals = matrix(0, nrow = length(PC_targs), ncol = length(TARGET_FEATURE_NAMES),
                dimnames = list(PC_targs, TARGET_FEATURE_NAMES))
        ...
        for (job in JOBS) {
            PC_LOADINGS = xhmm_data[[job]][["PC_LOADINGS"]]
            PC_LOADINGS = PC_LOADINGS[1:(nrow(PC_LOADINGS) -
                1), ]
            PC_LOADINGS_samps = colnames(PC_LOADINGS)
            PC_LOADINGS_t = t(PC_LOADINGS)
            rm(PC_LOADINGS)
            vals = matrix(0, nrow = length(PC_LOADINGS_samps),
                ncol = length(SAMPLE_FEATURE_NAMES), dimnames = list(PC_LOADINGS_samps,
                  SAMPLE_FEATURE_NAMES))
            for (sampVal in SAMPLE_FEATURE_NAMES) {
                vals[, sampVal] = xhmm_data[[job]][[sampVal]][PC_LOADINGS_samps]
            }
            PC_LOADINGS.corr[[job]] = rbind(cor(PC_LOADINGS_t,
                vals, use = "na.or.complete"), rep(0, length(SAMPLE_FEATURE_NAMES)))
        }
        absCorrWithPC = list()
        ...
        for (job in JOBS) {
            ...
            absCorrWithPC[[job]] = abs(cbind(PC.corr[[job]],
                PC_LOADINGS.corr[[job]]))
            ...
            ylim = c(0, 1)
            numPCsToPlot = c(numPC, 25, 50, 100, 150, 200, 250,
                300, 350, 400, 450, 500)
            for (plotPC in numPCsToPlot) {
                ...
                matplot(indsToPlot, corrsToPlot, ylim = ylim,
                  pch = pch, type = "p", col = colors, cex = PT_EXPANSION_FACTOR,
                  xlab = "Principal component", ylab = "Absolute value of correlation to PC scores/loadings",
                  main = paste(job, "", sep = ""))
                plotY = seq(from = ylim[1], to = ylim[2], length.out = 20)
                points(rep(NUM_PC_REMOVED, length(plotY)), plotY,
                  col = "black", type = "p", pch = "|")
                legend("top", featureNames, pch = pch, col = colors,
                  pt.cex = PT_EXPANSION_FACTOR)
            }
        }
        dev.off()
        ...
        for (job in JOBS) {
            ...
            heatmap.2(absCorrWithRemovedPC, Rowv = NA, Colv = NA,
                dendrogram = "none", scale = "none", revC = FALSE,
                density.info = "none", trace = "none", xlab = "",
                main = title, symbreaks = FALSE, colsep = 1:ncol(absCorrWithRemovedPC),
                sepcolor = "white")
        }
        dev.off()
        ...
        for (job in JOBS) {
            ...
            plot(useInds, SDdata[useInds], ylim = ylim, log = "y",
                xlab = "Principal component", ylab = "Standard deviation in component",
                main = title)
            NUM_PC_REMOVED = xhmm_data[[job]][["NUM_PC_REMOVED"]]
            plotY = exp(seq(from = log(ylim[1]), to = log(ylim[2]),
                length.out = 20))
            points(rep(NUM_PC_REMOVED, length(plotY)), plotY,
                col = "black", type = "p", pch = "|")
        }
        ...
        for (job in JOBS) {
            ...
            plotNames = c()
            for (i in 1:numPC) {
                plotName = paste(PLOT_PATH, "/PC/PC.", job, "_",
                  i, sep = "")
                plotNames = c(plotNames, plotName)
                title = paste("PC ", i, ": ", sep = "")
                largeCorrelationNames = colnames(absCorrWithPC[[job]])[which(absCorrWithPC[[job]][i,
                  ] >= CORR_THRESH)]
                if (length(largeCorrelationNames) == 0) {
                  title = paste(title, "no correlations >= ",
                    CORR_THRESH, sep = "")
                }
                else {
                  largeCorrelations = round(absCorrWithPC[[job]][i,
                    largeCorrelationNames], CORR_NUM_DECIMAL_PLACES)
                  title = paste(title, paste(largeCorrelationNames,
                    ": ", largeCorrelations, collapse = ", ",
                    sep = ""), sep = "")
                }
                plotAllChromosomeValues(chr, PC[i, ], "Projection to prinicipal component",
                  title, plotName)
            }
            if (system("which convert", ignore.stdout = TRUE,
                ignore.stderr = TRUE) == 0) {
                system(paste("convert ", paste(paste(plotNames,
                  ".png", sep = ""), collapse = " "), " ", PLOT_PATH,
                  "/PC.", job, ".pdf", sep = ""))
            }
        }
    }
            ...
            if (nrow(XCNV_CALLS) > 0) {
                for (cnvInd in 1:nrow(XCNV_CALLS)) {
                  ...
                  if (!is.null(INCLUDE_PEDIGREE_SAMPLES)) {
                    includeRows = which(INCLUDE_PEDIGREE_SAMPLES[["pedigree"]][,
                      "#ID"] == sample | INCLUDE_PEDIGREE_SAMPLES[["pedigree"]][,
                      "PAT"] == sample | INCLUDE_PEDIGREE_SAMPLES[["pedigree"]][,
                      "MAT"] == sample)
                  ...
                  for (addTargs in c(0, NUM_ADD_TARGS)) {
                    start_stop_inds = regionToStartStopInds(xhmm_data[[job]],
                    ...
                    plot_XHMM_targets(usePlotName, xhmm_data[[job]],
                      xhmm_data[[job]][["TARGETS_TO_GENES"]],
