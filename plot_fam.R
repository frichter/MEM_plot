...
sum(TARGETS_CHR_BP1_BP2$bp1 >= 1337395 &
    TARGETS_CHR_BP1_BP2$bp2 <= 1431087 &
    TARGETS_CHR_BP1_BP2$chr == "chr1")


plot_prefix = "/hpc/users/richtf01/chdiTrios/Felix/xhmm_test/plot_fam/1p36/y_upper5/fam"
PlotFamMEXHMM(sample_list[[1]], "chr1", 1337395, 1431087,
  dataList, TARGETS_CHR_BP1_BP2, plot_prefix)
lapply(sample_list[2:5], function(x)
  PlotFamMEXHMM(x, "chr1", 1337395, 1431087, dataList, TARGETS_CHR_BP1_BP2, plot_prefix))

# plotting all individuals on 1 graph, without parents
# run PlotFamMEXHMM line-by-line
...
PlotSampleList = function(sample_list, CHROM, START, END, dataList,
    ...
    chr = CHROM
    start_bp = 1337395 #START #- 50000
    end_bp = 1431087 #END #+ 50000
    type = "DEL"
    chr = "chr1"
...
plot_prefix = "/hpc/users/richtf01/chdiTrios/Felix/xhmm_test/plot_fam/1p36/proband_only"
PlotSampleList(sample_list, "chr1", 1337395, 1431087, dataList,
               TARGETS_CHR_BP1_BP2, plot_prefix, basename)

# identify probands with data in target regions 7-12
# Extract read depth z-scores for these individuals
sample_df = dataList[["PCA_NORM_Z_SCORES"]][, target_indices[7:12]]
sample_df = sample_df %>% as.data.frame %>% mutate(Blinded.ID = row.names(sample_df))

# What is the definition of "low"?
# use highest z-score in MEM probands + 0.5 per exon as cutoff
sample_list = c("1-02711", "1-04652", "1-05084", "1-06498", "1-07442")
blinded_id_interest = sample_df$Blinded.ID %in% sample_list
perColCutOffs = apply(sample_df[blinded_id_interest, 1:6], 2, max)

# count the number of targets less than the cut-off for probands and parents
id_index_to_keep = grep("-01$|-02$", row.names(dataList[["PCA_NORM_Z_SCORES"]]))
cutCount_vec = apply(sample_df[, 1:6], 1, function(pca_values) sum(pca_values <= perColCutOffs + 0.5))
sample_df$cutCount_empirical = cutCount_vec

sample_lo_list_proband = sample_df %>%
  filter(!grepl("-01$|-02$", Blinded.ID)) %>%
  filter(cutCount_empirical >= 6 & `chr1:1387418-1387724` < -2) %>%
...
plot_prefix = "/hpc/users/richtf01/chdiTrios/Felix/xhmm_test/plot_fam/1p36/parent_lo"
PlotSampleList(sample_lo_list, "chr1", 1337395, 1431087, dataList,
               TARGETS_CHR_BP1_BP2, plot_prefix, basename)
